      ...
      if(document.querySelector('#app-booking-form')) {
        let bookingForm = new Vue({
          ...
          methods: {
            revealSection() {
              ...
              if(this.selectedState) {
                visibleStep = 2;
                if (this.selectedRegion && this.selectedTown && this.selectedRadioStationId && this.selectedTimeSlot && this.selectedAdType) {
                  visibleStep = 3;
                }
              }
            ...
            getSortedRadioStations() {
              ...
              if(this.radioStations.length === 1) {
                ...
                this.selectedRadioStationId = this.radioStations[0][ID];
            ...
            updateRadioStation() {
              // this.selectedTimeSlot = '';
              // this.selectedAdType = '';
              if (this.selectedState && this.selectedRegion && this.selectedTown && this.selectedRadioStationId) {
                //todo
              }
              this.revealSection();
            },
            ...
            getRecordedAdvertisingItem(index) {
              if(this.savedRecordedAdvertisingItems.length > index) {
                this.currentRecordedAdvertisingItem = this.savedRecordedAdvertisingItems[index];
              }
            },
            setRecordedAdvertisingItem(recordedAdvertisingItem) {
              this.currentRecordedAdvertisingItem = recordedAdvertisingItem;
            },
            saveRecordedAdvertisingItem() {
              if(JSON.stringify(this.currentRecordedAdvertisingItem) !== JSON.stringify(this.generateRecordedAdvertisingItem())) {
                this.currentRecordedAdvertisingItem.station = {
                  state: this.selectedState,
                  region: this.selectedRegion,
                  town: this.selectedTown,
                  radioStation: this.selectedRadioStationId,
                  timeSlot: this.selectedTimeSlot
                }
                this.savedRecordedAdvertisingItems.push(this.currentRecordedAdvertisingItem);
                //if local storage
                // if(window.localStorage) {
                //   window.localStorage['savedRecordedAdvertisingItems'] = JSON.stringify(this.savedRecordedAdvertisingItems);
                // }
                // console.log(this.savedRecordedAdvertisingItems)
                this.currentRecordedAdvertisingItem = this.generateRecordedAdvertisingItem();
              }
            },
            removeRecordedAdvertisingItem(index) {
              if (this.savedRecordedAdvertisingItems.length > index) {
                this.savedRecordedAdvertisingItems.splice(index, 1);
              }
            },
            generateRecordedAdvertisingItem() {
              ...
              for(const prop in item.sessions) {
                item.sessions[prop]['week'] = {
                  monday: {
                    qty: ''
                  },
                  tuesday: {
                ...
                item.sessions[prop]['activeWeeks'] = '';
                item.sessions[prop]['weekCommencing'] = '',
                item.sessions[prop]['estimate'] = '';
                item.sponsorship = {
                  news: {
                    checked: false
                  },
            ...
            getMondays(limit) {
              ...
              for (var i = 0; i < limit - 1; i++) {
                let newMonday = new Date(d);
                newMonday.setDate(d.getDate() + 7);
                d.setDate(d.getDate() + 7);
                dates.push(newMonday);
              }
              return dates;
            },
            formatDate(format, date) {
              const DAYS = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
              const DAYS_ABBREVIATED = ['Sun', 'Mon', 'Tue', 'Wed', 'Thurs', 'Fri', 'Sat'];
              ...
              if(format === 'ddmmyy') {
                ...
                let dayName = DAYS_ABBREVIATED[date.getDay()];
                let day = this.suffixNumber(date.getDate());
                let monthName = MONTHS_ABBREVIATED[date.getMonth()];
                let year = date.getFullYear();
            ...
            abbreviateDay(name) {
              ...
              if (name === 'monday') return 'Mon';
              if (name === 'tuesday') return 'Tue';
              if (name === 'wednesday') return 'Wed';
              if (name === 'thursday') return 'Thur';
              if (name === 'friday') return 'Fri';
              if (name === 'saturday') return 'Sat';
              return name;
            },
            suffixDay(int) {
              if(int === 1 || int === 21 || int  === 31) return int + 'st';
              if(int === 2 || int === 22) return int + 'nd';
              if(int === 3 || int === 23) return int + 'rd';
              return int + 'th';
            },
            suffixNumber(number){
              //by KB
              let reg = number.toString().match(/^((\d*)?\d{1,2})$/im);
              return number+((reg[2]==1) ? 'th': (reg[1].substr(-1) == 3) ? 'rd':(reg[1].substr(-1) == 2) ? 'nd': (reg[1].substr(-1) == 1) ? 'st':'th');
            },
            updatedSponsorshipOther() {
              if(this.currentRecordedAdvertisingItem.sponsorship) {
                this.currentRecordedAdvertisingItem.sponsorship.other.checked = !this.currentRecordedAdvertisingItem.sponsorship.other.checked;
                if(this.currentRecordedAdvertisingItem.sponsorship.other.checked) {
                  this.currentRecordedAdvertisingItem.sponsorship.otherText.disabled = false;
                } else {
                  this.currentRecordedAdvertisingItem.sponsorship.otherText.disabled = true;
                  this.currentRecordedAdvertisingItem.sponsorship.otherText.text = '';
                }
              }
            },
            log(str) {
          ...
          beforeMount() {
            ...
            console.log(this.marketData)
            this.sessionTimes = station_data || {};
            console.log(this.sessionTimes)
